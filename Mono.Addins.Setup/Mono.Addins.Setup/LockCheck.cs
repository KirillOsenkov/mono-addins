...
namespace Mono.Addins.Setup
{
	class LockCheck
	{
		...
		public class ProcessInfo
		{
			internal ProcessInfo (RM_PROCESS_INFO processInfo)
			{
				...
				if (string.IsNullOrWhiteSpace (ApplicationName)) {
					ApplicationName = Process.GetProcessById (ProcessId).ProcessName;
				}
		...
		public static string GetProcessesLockingFiles (string[] filePaths)
		{
			try {
				return string.Join (", ", GetLockingProcessInfos (filePaths).Select (p => $"{p.ApplicationName} ({p.ProcessId})"));
			} catch (Exception ex) {
				return ex.ToString ();
			}
		}

		public static string GetProcessesLockingFolder (string folderPath, string searchPattern = null, bool recursive = false)
		{
			try {
				searchPattern = searchPattern ?? "*";
				var files = Directory.GetFiles (folderPath, searchPattern, recursive ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly);
				var lockers = GetProcessesLockingFiles (files);
				return lockers;
			} catch (Exception ex) {
				return ex.ToString ();
			}
